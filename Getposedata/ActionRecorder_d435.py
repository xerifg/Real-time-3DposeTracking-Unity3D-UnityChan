...
class ActionRec(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def transtoPose17(self, pose3D):
        ...
        for i in range(3):
            if i == 0:
                datapose17[0] = [center[i], pose3D[24][i], pose3D[26][i], pose3D[28][i], pose3D[23][i], pose3D[25][i],
            ...
            if i == 1:
                datapose17[2] = [center[i], pose3D[24][i], pose3D[26][i], pose3D[28][i], pose3D[23][i], pose3D[25][i],
                                 pose3D[27][i], spine[i], chest[i], neck[i],
                                 head[i], pose3D[11][i], pose3D[13][i], pose3D[15][i], pose3D[12][i], pose3D[14][i],
                                 pose3D[16][i]]
    ...
    def play(self):
        ...
        ret, depth_frame, self.originFrame = self.capture.get_frame()
        if (ret == True):
            """####################  preprocess  ########################"""
            ...
            # saverawimage(img_pose)
            # # create pose estimator
            # image_size = self.originFrame.shape
            #
            ...
            # try:
            #     # estimation
            #     pose_2d, visibility, self.pose_3d = pose_estimator.estimate(self.originFrame)
            #
            #     # Show 2D and 3D poses
            #     self.display_results(self.originFrame, pose_2d, visibility, self.pose_3d)
            # except ValueError:
